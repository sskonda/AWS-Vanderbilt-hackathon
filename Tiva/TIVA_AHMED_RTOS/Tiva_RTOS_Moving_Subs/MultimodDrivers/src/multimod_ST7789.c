...
void ST7789_SetWindow(int16_t x, int16_t y, int16_t w, int16_t h) {
    ...
    if(((x + w) > X_MAX) || ((y + h) > Y_MAX) || (x < 0) || (y < 0)){
        return;
    }

    // Set column address
    ST7789_WriteCommand(ST7789_CASET_ADDR);
    ST7789_WriteData(x >> 8);
    ST7789_WriteData(x);
    ST7789_WriteData((x + w -1) >> 8);
    ST7789_WriteData(x + w -1);
    // Set row address
    ST7789_WriteCommand(ST7789_RASET_ADDR);
    ST7789_WriteData(y >> 8);
    ST7789_WriteData(y);
    ST7789_WriteData((y + h - 1) >> 8);
    ST7789_WriteData(y + h - 1);
